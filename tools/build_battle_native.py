"""Compile the audited battle geometry routines from an owned FIGHTSTG.PRO.
...
MODULE_SHA256 = '92b6168a3d611bfc98e1c3da199d422dd05f76f6269496e393347155d52f7ab8'
BASE = 0x80082cb0
END = 0x800a0000
ROOT = 0x80085110
EXPECTED_RANGES = {(0x847f4, 0x260), (0x84a54, 0x16c), (0x84bc0, 0x24c),
                   (0x84e0c, 0x1f4), (0x85000, 0x110), (0x85110, 0x848)}


def capture(module):
    if hashlib.sha256(module).hexdigest() != MODULE_SHA256:
        raise ValueError('Unsupported FIGHTSTG.PRO; extract it from the audited European disc')
    ...
    return dict(schema='psxrecomp overlay capture v2', load_addr=hex(BASE),
                ...
                executed_pcs=[hex(ROOT)], dispatch_entry_pcs=[hex(ROOT)],
                function_entry_pcs=[hex(ROOT)], seeds=[hex(ROOT)])
...
def validate_ranges(source):
    ...
    if ranges != EXPECTED_RANGES:
        raise ValueError('Generated geometry coverage changed; review before enabling')
...
def build(args):
    ...
    (args.output/'provenance.json').write_text(json.dumps(dict(
        module_sha256=MODULE_SHA256, root=hex(ROOT),
