...
try {
    Invoke-Checked $Python @('tools/audit_disc.py', $disc, '--output', 'extracted/audit', '--extract-exe')
    ...
    Invoke-Checked 'cmake' @('--build', 'build-windows', '--config', 'Release', '--target', 'shinka', '--parallel', "$Jobs")
} finally {
    Pop-Location
}
