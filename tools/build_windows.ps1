...
try {
    ...
    Invoke-Checked 'cmake' @('--build', 'build-windows', '--config', 'Release', '--target', 'shinka', 'shinka_overlay_guard_test', 'shinka_journal_menu_test', 'shinka_menu_exp_test', 'shinka_evolution_chart_test', 'shinka_encounters_test', 'shinka_map_travel_test', 'shinka_dev_nav_test', 'shinka_music_live_test', '--parallel', "$Jobs")
    Invoke-Checked 'ctest' @('--test-dir', 'build-windows', '-C', 'Release', '--output-on-failure', '-R', '^(overlay_guard|journal_menu|menu_exp|evolution_chart|encounters|map_travel|dev_nav|music_live)$')
} finally {
    Pop-Location
}
