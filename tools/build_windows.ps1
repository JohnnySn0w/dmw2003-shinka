...
try {
    Invoke-Checked $Python @('tools/audit_disc.py', $disc, '--output', 'extracted/audit', '--extract-exe', '--extract-file', 'STFGTREP.PRO', '--extract-file', 'STGDGLAB.PRO', '--extract-file', 'FIELDSTG.PRO')
    ...
    Invoke-Checked 'cmake' @('--build', 'build-windows', '--config', 'Release', '--target', 'shinka', 'shinka_overlay_guard_test', 'shinka_journal_menu_test', 'shinka_menu_exp_test', 'shinka_evolution_chart_test', 'shinka_encounters_test', '--parallel', "$Jobs")
    Invoke-Checked 'ctest' @('--test-dir', 'build-windows', '-C', 'Release', '--output-on-failure', '-R', '^(overlay_guard|journal_menu|menu_exp|evolution_chart|encounters)$')
} finally {
    Pop-Location
}
